---
name: tdd
description: "TypeScript regression-testing workflow for an explicit request for TDD, a failing test, or a regression test. Demonstrate the intended failure before fixing and a pass afterward; use the closest useful verification when a meaningful test is impractical."
...
Use the TypeScript project's existing test runner, scripts, and test location.
For a stateless class, test its public behavior with per-call inputs and stable
dependencies; do not replace its architecture to make the test easier. React
tests should exercise observable user behavior. This skill retains the source's
explicit invocation policy; ordinary verification remains part of project rules.
