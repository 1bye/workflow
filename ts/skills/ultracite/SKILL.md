...
Use this guidance for TypeScript projects using Ultracite or explicitly adopting
it. Existing project instructions and effective formatter configuration govern
style. Preserve stateless domain classes, static-only classes, and constructor
parameter properties when the project allows them; React components stay functions.

Prefer existing project scripts and installed binaries. Check what a script does
before running it: some projects call a fixing command `check`. Initializers,
dependency upgrades, hooks, agent files, and removal of old configs belong to a
requested tooling migration. Do not run them as routine lint troubleshooting.
Apply framework-specific examples only to that framework and installed version.
Own promises by awaiting, returning, or using an explicit handled background task.
